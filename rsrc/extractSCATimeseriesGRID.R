...
sca_wd = args[2]
# sptaialsubset=
# raster('/home/joel/sim/ensembler_scale_sml/ensemble0/grid9/landform.tif')
...
# now stack cropped data in simdir
setwd(paste0(simdir,"/modis"))

if (length(list.files(pattern = "MOD*")) > 0) {
    print(paste0(length(list.files(pattern = "MOD*")), " MOD files found"))
    MOD = stack(list.files(pattern = "MOD*"))
    print("MOD stack crop complete")
    MOD.names = names(MOD)
}
if (length(list.files(pattern = "MYD*")) > 0) {
    print(paste0(length(list.files(pattern = "MYD*")), " MYD files found"))
...
if (length(myd.missmod.ind) > 0) {
    # stack MOD and new MYD fill layers and index by MOD nomiss layer index and MOD
    # miss layer index
    MOD.layerfill <- subset(stack(MOD, MYD[[myd.missmod.ind]]), order(c(MOD.nomiss.ind,
        MOD.miss.ind)))
...
if (length(mod.missmyd.ind) > 0) {
    # stack MOD and new MYD fill layers and index by MOD nomiss layer index and MOD
    # miss layer index
    MYD.layerfill <- subset(stack(MYD, MOD[[mod.missmyd.ind]]), order(c(MYD.nomiss.ind,
        MYD.miss.ind)))
...
# MOD Na cells filled with MYD data if exists : Replace ‘NA’ values in the first
# Raster object (‘x’) with the values of the second (‘y’), and so forth for
# additional Rasters.
...
print(paste0("orig NAs in MOD=", round(oldNa, 2), "% New NAs in filled MOD=", round(newNa,
    2), "%"))
...
# Combine timeseries MOD[MOD > 100] <- NA MYD[MYD > 100] <- NA

# DOesnt work for grids my.na <- is.na(MOD) MOD[my.na] <- MYD[my.na]
...
# ==================================================================== MODIS SA
# CODES ====================================================================
# 0-100=NDSI snow 200=missing data 201=no decision 211=night 237=inland water
