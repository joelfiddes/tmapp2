...
meanswe =read.csv(meanVar ,head=F )
	...
	for (i in 1:length(gridseq)){
		...
		writeRaster(rst, paste0(tsub_root, "/g",grid,"_swe",outname,".tif"),overwrite=TRUE)
		}
...
rasters1 <- list.files(tsub_root, pattern=paste0("*swe",outname,".tif"),full.names=TRUE, recursive=TRUE)
rast.list <- list()
